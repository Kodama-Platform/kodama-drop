...
## Update (2026-06) — Landing redesign (explain-in-a-minute hero)
- Root landing (`talk-surface.tsx` idle branch) rebuilt into an asymmetric two-column hero: LEFT = leaf mark + Fraunces headline "One address. Every conversation." + subline + the live address field + status; RIGHT = an editorial explainer (`landing-hero.tsx`) — DOM-recreated diagram (three Drop note-cards → dotted arrows → "YOUR TALK ADDRESS" slab reflecting the typed slug) + a mini "A Drop can become a Talk" trail timeline + a 3-beat explainer (A Drop / You decide / It stays yours).
- The explainer column fades out (opacity+blur+translate, `.landing-explainer--dim`) the moment the user types (status !== idle), so it never blocks the live in-place morph (Door / Claim / Open my Talk reveal via SceneFade). Left column morphs into the place preview (mark + name) while resolving.
- Atmospheric forest-mist backdrop (`LandingBackdrop`, fixed, theme-aware): `/public/landing/mist-light.jpg` (light) + `mist-dark.jpg` (dark), low opacity with a background-tone vignette so text stays readable. Layered above the existing ForestAtmosphere fireflies.
- No security/encryption claims. Light + dark both verified via screenshots (idle, typing→Door reveal, dark idle). 158/158 unit tests pass; entry-flow testids unchanged (landing-title, address-plaque-input, landing-reveal, open-my-talk, reserved-state).
- Design blueprint: `/app/design_guidelines.json` (adapted — real source is `/app/src`, CSS transitions instead of Framer Motion to avoid the Node>=22 dep issue).

## Update (2026-06) — Landing v2: app snapshots as the visual (replaces forest photo + text)
- Per user feedback ("forest background image is not useful; I wanted snapshots as background, not text explanation"): removed the forest-mist photo backdrop AND the text-heavy editorial diagram/3-beat explainer.
